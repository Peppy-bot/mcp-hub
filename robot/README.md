...
The [launchers hub](https://github.com/Peppy-bot/launchers-hub) serves the exposure as one server
per clock: `robot_control` on wall time for the physical robots and `robot_control_sim` on the
simulation's clock for the simulated ones (`mcp/fragments/robot_control.json5` and
`robot_control_sim.json5`). A simulation launcher runs the second alone; the fleet runs either or
both, and a fleet of real robots beside a simulation serves two URLs, 8900 and 8901. Every robot
beside the server of its clock is listed on it with its identity and limb state, and with its
brain and recorder whenever they run; its `mcp_commander` option adds the backbone's moves, and
its camera rig adds the cameras under that option, `cameras` on hardware and `cameras_sim` in
simulation, so a robot without a rig enrolls with no camera. A join adds its robot to the running
server and a removal takes it out:

```sh
peppy stack launch openarm_simulation_mcp --join openarm_v2_sim:alpha                       # Waldo, beside the simulated world's endpoint
peppy stack launch openarm_simulation_mcp --join openarm_v2_sim:alpha --join so101_sim:charlie   # an OpenArm and an SO-101 on the one URL
peppy stack join openarm_v2_sim -i bravo                                                    # listed when the join returns
peppy stack launch openarm_simulation_mcp --join openarm_v2_sim:alpha --with mujoco,simulation_mcp=none   # MuJoCo, this endpoint alone
peppy stack launch fleet --with robot_control
peppy stack join openarm_v2 -i alpha --with mcp_commander,cameras                           # the real robot
peppy stack launch fleet --with waldo,robot_control,robot_control_sim                       # real robots on 8900, simulated ones on 8901
```
...
model to expect. On hardware the `uvc_camera` and `zed_camera` nodes describe no profile or
geometry, so those tools refuse for their cameras and the listing's capabilities say so. The first
launch also serves the simulated world's own endpoint,
